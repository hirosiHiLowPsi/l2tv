...
LR2の段位コースで `score.db` の `clear=2` として保存されるノーマル合格は、通常譜面のEASY CLEARではなく段位の `NORMAL CLEAR` 相当として扱います。

GENOSIDE2018 SP段位、Stella Skill Simulator 4th、Satellite Skill Analyzer 2nd は、同梱した `.lr2crs` 由来のコースhashも照合に使用します。これにより、`song.db` の `grade` テーブルに欠落や表記差がある環境でも、段位コースのクリア記録を拾いやすくします。

発狂段位コースは、コースのEX SCORE RATEが取得できる場合、段位定数にもスコア係数を掛けます。

発狂段位定数は、LR2IR Archive のGENOSIDE2018段位ランキングを参照し、プレイ人数に対して「段位合格」と「AAA以上」の両方を達成している人数の割合を主な基準にします。発狂段位は単曲レベルとは別の到達目標として扱うため、発狂段位専用のベース値にAAA希少度補正を加えます。

ただし、GENOSIDE2018 Overjoy はスコア係数を掛けません。AAAが存在しない性質の段位として扱い、合格した時点で段位定数満額です。
...
```text
発狂段位FORCE = 段位定数 × 段位スコア係数
GENOSIDE2018 Overjoy FORCE = 段位定数
```

段位スコア係数は通常譜面と同じスコア係数を使用します。コースEX SCORE RATEを取得できない場合は、互換性維持のため `1.00` として扱います。
