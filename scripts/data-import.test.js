...
test("score, lamp, and URL safety helpers reject unsafe or invalid values", () => {
  ...
  assert.equal(isBlockedIpAddress("fec0::1"), true);
...
test("profile DB work runs through the worker without enabling Stellaverse access", async () => {
  await withTempDirectory(async (directory) => {
    const scoreDbPath = path.join(directory, "190114.db");
    const songDbPath = path.join(directory, "song.db");
    createDatabase(songDbPath, [
      "CREATE TABLE grade (title TEXT, hash TEXT)",
    ]);
    createDatabase(scoreDbPath, [
      "CREATE TABLE player (id TEXT, name TEXT, irid TEXT, grade_7 INTEGER, grade_14 INTEGER, playtime INTEGER)",
      "INSERT INTO player VALUES ('190114', 'Offline Name', '190114', 0, 0, 10)",
      "CREATE TABLE score (hash TEXT, scorehash TEXT, clear INTEGER, playcount INTEGER, perfect INTEGER, great INTEGER, totalnotes INTEGER)",
    ]);
    ...
    try {
      const config = await request(port, "/api/client-config");
      const token = JSON.parse(config.body).apiToken;
      const response = await request(port, "/api/profile-from-db", {
        method: "POST",
        headers: {
      ...
      assert.equal(payload.player.name, "Offline Name");
      assert.equal(payload.player.lr2Id, "190114");
    } finally {
      await new Promise((resolve, reject) => appServer.close((error) => (error ? reject(error) : resolve())));
    }
  });
