...
- 難易度表一覧と選択した難易度表の取得時にインターネット通信を行います。
- Stellaverse IR通信は初期状態でOFFです。利用者がメニューで明示的に許可した場合だけ、プレイヤーIDを送信して名前・順位・公開スコアを取得します。
- 配布版はElectron版のみです。
...
- StellaverseIR選択時でも、メニューでStellaverse IR接続を許可した場合に限り、`score.db` 内のIDを使ってプレイヤー名取得を試みます。
...
- IR順位とIR状況は個別にON / OFFできます。Stellaverse IR接続がOFFの場合は、プロフィール・順位・ライバルの通信取得を行いません。
- 各列を昇順・降順で並べ替えられます。
...
- LR2系の `score.db`
- LR2系の `song.db`
- インターネット接続（難易度表一覧・難易度表・Stellaverse IR機能を利用する場合）
...
Electron版とTauri版は保存領域が異なるため、専用JSONを使用します。

1. 従来の`lr2ir-table-lamp-viewer-data`フォルダが隣にある場所から、引継ぎ対応済みElectron版を起動します。
2. メニューの`その他`で`引継ぎデータを書き出す`を押します。
3. Tauri版を起動し、同じ画面の`引継ぎデータを読み込む`からJSONを選択します。
4. 上書きを確認すると、設定、難易度表、Stellaverse Rival ID、前回解析結果が移行されます。

この操作は`score.db`や`song.db`をコピー・変更しません。Tauri版でも元のDBパスを読み取り専用で参照します。
...
- LR2 / OpenLR2のDBは読み取り専用で開き、更新・削除・移動しません。
- DB処理は専用Workerで実行し、Electronの画面処理から分離しています。
- ローカルAPIはループバック専用の固定オリジンを使い、ポートを自プロセスで確保できたことを確認してから画面を開きます。Host・Origin・ランダムトークンも検証します。
- カスタム難易度表URLは、DNS解決後もローカル・プライベート・予約済みIPへの接続を拒否します。
- Stellaverse IR通信は初期OFFです。ONにした場合、プレイヤーIDを `https://ir.stellabms.xyz/` へ送信します。
- Stellaverse IRへDBファイル、DBパス、ローカルスコア全体をアップロードしません。
- IR取得は順番待ち、アクセス間隔、キャッシュ、件数上限を設けて負荷を抑えています。
- DBパス、表示設定、直前の解析結果はWindowsユーザーデータ領域へ保存され、暗号化はされません。同じWindowsユーザーから隠す用途には使えません。メニューの「保存データを消す」で削除できます。
- L2TVは未署名です。GitHub Releasesの配布元と同梱のSHA256ファイルを確認してください。

脆弱性を見つけた場合は、公開Issueへ機密情報を書かず、GitHubのPrivate vulnerability reportingから連絡してください。詳細は [SECURITY.md](SECURITY.md) を参照してください。
...
6. Stellaverse IRの名前・順位・公開スコアを取得する場合だけ、「Stellaverse IRへの接続を許可する」をONにします。
7. ライバル比較を使う場合は、Rivalフォルダを指定します。
8. 「一覧を開く」から難易度表一覧を開き、読み込む表にチェックを入れます。
9. 必要に応じて「追加済み」を開き、ドラッグで表示順を変更します。
10. 「表とランプを読み込む」を押します。
...
Windows向け7z配布アーカイブとSHA256ファイルを作成する場合:
...
server.js             ローカルAPIサーバーとデータ処理
database-worker.js    DB読み取り用Worker
