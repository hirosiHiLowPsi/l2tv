...
- β機能として、LR2ID一致でLR2IRランキング上の自分の行を手動取得
...
- LR2IRランキング取得βは既定OFFです。利用時はメニューで有効化し、譜面一覧の `IR β取得` を押したときだけ現在表示中の最大25譜面を取得します。自分の特定はLR2IDとランキングページの `playerid` が一致した場合だけ行います。
